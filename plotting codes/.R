...
# COVID hierarchy
# hier<-get_location_metadata(location_set_id = 111, location_set_version=771, release_id=9)
# saveRDS(hier, "/home/j/temp/reed/misc/20210518_hier.RDS")
hier <- readRDS("/home/j/temp/reed/misc/20210518_hier.RDS")
bats<-fread('/snfs1/temp/ehulland/COVID-19/VCCCM/bats/bat_covariate_new_log5.csv')
cancer<-fread('/snfs1/temp/ehulland/COVID-19/VCCCM/prepped_covariates_rmbarber.csv')[,-1]
df_pop_in <- read.csv("/ihme/covid-19/model-inputs/best/output_measures/population/all_populations.csv")
df2 <- data.frame(location_id=rep(NA,206),govt_trust_corrupt_pca=rep(NA,206),govt_trust_pca=rep(NA,206),govt_corrupt_pca=rep(NA,206))
df2$location_id<-pca_trust_corrupt$data$location_id
df2$govt_trust_corrupt_pca<-pca_trust_corrupt$pca_mi_result[[num+1]]
df2$govt_trust_pca <- pca_govt_trust$pca_mi_result[[num+1]]*-1
df2$govt_corrupt_pca <-pca_corruption$pca_mi_result[[num+1]]*-1

#function for getting a draw:
get_yhat<-function(model_fit, ndraws, xmatrix){
  betas = coef(model_fit)
  vcov = vcov(model_fit)
  betas_simulated = MASS::mvrnorm(ndraws, betas, vcov) #make sure to get the draw this way for stage 2 betas!!!
  beta_mat<-t(as.matrix(betas_simulated))
  yhat = beta_mat %*% xmatrix
  return(yhat)
}

dir.create(paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/"))

# process analytic datasets

prep_stage1_data <- function(csv_path, wave_id) {
  if (FALSE) {
    csv_path <- "/snfs1/temp/ehulland/COVID-19/VCCCM/final_df_wave1_covariates_scaled_subnats.csv"
    wave_id <- 1
  }
  ...
  final_df_tmp <- merge(final_df_tmp, bats, by='location_id') # drops location_id values: 19, 90, 4850 (Timor-Leste, Norway, Goa)
  final_df_tmp <- merge(final_df_tmp, cancer, by='location_id') # drops location_id values: 19, 90, 4850 (Timor-Leste, Norway, Goa)
...
final_df_list <- lapply(0:4, function(x) {
  csv_path_tmp <- paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version,"/", experiment,"/draws/final_df_wave", x, "_covariates_scaled_", draw,".csv")
  prep_stage1_data(csv_path = csv_path_tmp, wave_id = x)
})

# -- seroprev locations only
seroprev<-fread('/ihme/covid-19-2/model-inputs/best/serology/global_serology_summary.csv') 
...
run_stage1 <- function(data_object, depvar, covars) {
  if (FALSE) {
    data_object <- final_df_list[[1]]
    data_object <- final_df_list_seroprev[[1]]
    depvar <- "log_period_ifr_age_adjusted"
    covars <- c("BMI", "proportion_1000ppl_km", "smoke_mean", "air_pollution", "summed_bat_species", "cancer", 'copd')
  }
  ...
  fwrite(vifs, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "/draws/vifs_",depvar,"_", draw, ".csv"))
  ...
  #fwrite(betas_out, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/stage1_results_",version_id,"_", draw, ".csv"))
  ...
  if (FALSE) {
    summary(dat$obs_minus_pred)
    summary(dat$residual_function)
    hist(dat$obs_minus_pred, breaks = 20)
    hist(dat$residual_function, breaks = 20)
    # with(dat, plot(stage1_residual, stage1_residual_v2))
    with(dat, plot(obs_minus_pred, residual_function))
    grid()
  }
...
run_stage2 <- function(stage1_output, covars_stage2, covs_filepath, verbose = FALSE) {
  if (FALSE) {
    data_object <- final_df_list
    depvar <- "log_period_ifr_age_adjusted"
    covars_stage1 <- c("BMI", "proportion_1000ppl_km", "smoke_mean", "GDP_mean", "air_pollution", "summed_bat_species","cancer", 'copd')
    # covars_stage2 <- c("HAQI_mean", "uhc_19", "ghsi_0_overall", "ghsi_1_prevent", "ghsi_2_detect", "ghsi_3_respond", "ghsi_4_health_sector", "ghsi_5_commit", "ghsi_6_risk", "jee_readyscore_overall", "jee_readyscore_prevent", "jee_readyscore_detect", "jee_readyscore_respond", "jee_readyscore_other", "THEpc", "GHESpc", "gini", "bureaucracy_corrupt", "gov_effect", "v2x_mpi", "v2x_polyarchy", "state_fragility", "lib_cons_avg", "v2pariglef_gov_avg", "interpersonal_trust", "trust_science", "govt_trust", "trust_govt_cpi", "trust_govt_gallup", "trust_govt_pca", "electoral_pop", "federal_ind", "mobility_min", "testing_mean", "mean_fraction_mandate", "BedsPerCapita", "vax_coverage_max")
    # covars_stage2 <- c(HAQI_mean = "HAQI (scaled)", uhc_19 = "UHC (scaled)")
    covars_stage2 <- covars_stage2_v1
    
    stage1_output <- lapply(data_object, function(x) {
      run_stage1(data_object = x, depvar = depvar, covars = covars_stage1 )
    })[[1]]
    covs_filepath <- '/snfs1/temp/ehulland/COVID-19/VCCCM/prepped_covariates_20210423.csv'
  }
  ...
  try({
    ...
    dat_by_covar <- sapply(names(covars_stage2), function(covar) {
      ...
      #fwrite(betas_out, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/stage2_", covar,"_",version_id,"_", draw, ".csv"))
...
run_stage1_stage2 <- function(data_object, depvar, covars_stage1, covars_stage2, version_id) {
  if (FALSE) {
    data_object <- final_df_list
    depvar <- "log_period_ifr_age_adjusted"
    covars_stage1 <- c("BMI", "proportion_1000ppl_km", "smoke_mean", "air_pollution", "summed_bat_species","cancer", 'copd')
    # covars_stage2 <- c("govt_trust")
    covars_stage2 <- covars_stage2_v1
  }
...
#saveRDS(all_results, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/all_stage1_stage2_results_", draw,".RDS"))
...
fwrite(stage1_results, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/stage1_results_", draw, ".csv"))
...
fwrite(stage2_results, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "final_outputs/stage2_results_", draw, ".csv"))
...
fwrite(fig2, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "fig2_data_", draw, ".csv"))
...
fig2<-rbind(stage_1_dat1, stage_1_dat2, stage_1_dat3, stage_1_dat4)
fwrite(fig2, paste0("/ihme/scratch/users/ehulland/covid-crosscountry/", seir_version, "/", experiment, "/", "fig2_data_sero_", draw, ".csv"))
